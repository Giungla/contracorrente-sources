  ...
  try {
    const response = await fetch(`${XANO_BASE_URL}/api:ltSOgTnO/beacon`, {
      ...
      body: stringify({
        custom: payload,
        environment: {
          url: location.protocol.concat('//', location.hostname, location.pathname, location.search),
        },
      }),
      keepalive: true,
    })
...
const ContraCorrenteOrderPage = defineComponent({
  name: 'PIXProcessPage',
  ...
  /**
   * Ações executadas logo após a criação do componente
   */
  ...
  methods: {
    /**
     * Inicia o SSE que ficará ouvindo modificações no pedido vindas do backend
     */
    ...
    /**
     * Atualiza as informações do pedido parcialmente
     */
    ...
    /**
     * Captura inicial dos dados do pedido
     */
    ...
    async handleCopyQRCode (): Promise<void> {
      ...
      if (navigator.clipboard.writeText) {
        ...
        input.value = this.order?.qrcode_text ?? EMPTY_STRING
  ...
  computed: {
    ...
    /**
     * Indica se o pedido consta pago
     */
    hasPaid (): boolean {
    ...
    /**
     * Indica se o pedido consta expirado
     */
    isExpired (): boolean {
    ...
    /**
     * Retorna a linha copiável do QRCode
     */
    getQRCode (): string {
...
if (!pixProcessElement) {
  ...
  createApp(ContraCorrenteOrderPage).mount(pixProcessElement)
}
